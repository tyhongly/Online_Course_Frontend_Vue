...
const isFree = (course) =>
  course.type === "document" || Number(course.price) === 0;
const descriptionFor = (course) =>
  course.description ||
  ...
  >
    <div class="relative h-48 shrink-0 overflow-hidden">
      ...
      <div class="absolute bottom-4 left-4 flex flex-wrap gap-2">
        <span
          class="rounded-full bg-white/90 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-700 backdrop-blur"
        >
          {{ course.category }}
        </span>
        <span
    ...
    <div class="flex flex-1 flex-col p-5">
      <h3
        class="line-clamp-2 text-[1.02rem] font-semibold leading-6 text-slate-950 transition group-hover:text-sky-700"
      >
        {{ course.title }}
      </h3>
      ...
      >
        ...
        <span
          class="inline-flex items-center gap-1 text-xs font-semibold text-blue-700 transition group-hover:gap-2 group-hover:text-sky-500"
        >
          View course
