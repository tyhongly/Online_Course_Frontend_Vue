...
const lastActivity = computed(() =>
  user.value.lastActivityAt
    ? new Date(user.value.lastActivityAt).toLocaleString()
    : "Unavailable",
);
    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      >
        ...
        >
          <div>
            <div
              class="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-indigo-200"
            >
              <ShieldCheck class="h-3.5 w-3.5" /> Admin account
            </div>
            <h1 class="mt-5 text-3xl font-semibold tracking-tight sm:text-4xl">
              Profile settings
            </h1>
            <p class="mt-2 max-w-xl text-sm leading-6 text-slate-300">
              Manage the identity and account details used across your course
              platform.
            </p>
          </div>
      ...
      >
        <Check class="mr-2 inline h-4 w-4" />{{ message }}
      </div>
      ...
      <section class="grid gap-6 lg:grid-cols-[310px_minmax(0,1fr)]">
        ...
        >
          ...
          <div class="space-y-4 p-6 text-sm">
            ...
            <div class="flex items-center justify-between">
              ...
              ><span
                class="max-w-[150px] text-right font-medium text-slate-700"
                >{{ lastActivity }}</span
              >
            </div>
            <div
              v-if="user.bio"
              class="border-t border-slate-200 pt-4 leading-6 text-slate-600"
            >
              {{ user.bio }}
            </div>
          </div>
        ...
        <div class="space-y-6">
          <!-- <div class="grid gap-3 sm:grid-cols-3">
            ...
            >
              <div
                class="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.12em] text-slate-500"
              >
                <span>Account health</span
                ><Activity class="h-4 w-4 text-emerald-500" />
              </div>
              <div class="mt-2 text-lg font-semibold text-emerald-700">
                {{ user.isSuspended ? "Needs review" : "All clear" }}
              </div>
            </div>
            <div
              class="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_12px_30px_-25px_rgba(15,23,42,0.3)]"
            >
              ...
              <div class="mt-2 text-lg font-semibold capitalize text-slate-950">
                {{ user.role || "admin" }}
              </div>
            </div>
            <div
            ...
            >
              ...
              <div class="mt-2 truncate text-lg font-semibold text-slate-950">
                #{{ user.id || "—" }}
              </div>
            </div>
          </div> -->
