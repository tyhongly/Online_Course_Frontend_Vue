...
const wishlistRows = computed(() => {
  ...
  studentUsers.value.forEach((student) => {
    const ids =
      wishlistStore.wishlists[String(student.id)] ||
      wishlistStore.wishlists[student.email] ||
      [];
  ...
  return [...counts.entries()]
    ...
    .sort(
      (a, b) =>
        b.count - a.count || a.course.title.localeCompare(b.course.title),
    );
});

const totalSaved = computed(() =>
  wishlistRows.value.reduce((sum, row) => sum + row.count, 0),
);
const studentsWithWishlist = computed(
  () => new Set(wishlistRows.value.flatMap((row) => row.students)).size,
);
  ...
  <div class="min-h-full bg-grey-300 p-5 sm:p-8">
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <section class="grid gap-3 sm:grid-cols-3">
        <article
          class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_15px_35px_-28px_rgba(15,23,42,0.35)]"
        >
          <div
            class="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.14em] text-slate-500"
          >
            Total saves <Heart class="h-4 w-4 text-rose-500" />
          </div>
          <div
        ...
        >
          <div
            class="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.14em] text-slate-500"
          >
            Interested students <Users class="h-4 w-4 text-indigo-500" />
          </div>
          <div
            class="mt-3 text-3xl font-semibold tracking-tight text-slate-950"
          >
            {{ studentsWithWishlist }}
          </div>
          <p class="mt-2 text-sm text-slate-500">Students with saved courses</p>
        </article>
        <article
          class="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_15px_35px_-28px_rgba(15,23,42,0.35)]"
        >
          <div
            class="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.14em] text-slate-500"
          >
            Top course <BarChart3 class="h-4 w-4 text-emerald-500" />
          </div>
          <div class="mt-3 truncate text-lg font-semibold text-slate-950">
            {{ mostWanted?.course.title || "No data yet" }}
          </div>
          <p class="mt-2 text-sm text-slate-500">
            {{
              mostWanted
                ? `${mostWanted.count} student saves`
                : "Students have not saved courses yet"
            }}
          </p>
        </article>
      </section>
      ...
      <section
        class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_45px_-34px_rgba(15,23,42,0.3)]"
      >
        <div
          class="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-6"
        >
          <div>
            <h2 class="font-semibold text-slate-950">Most wanted courses</h2>
            <p class="mt-1 text-sm text-slate-500">
              Courses ranked by student wishlist saves
            </p>
          </div>
          <BookHeart class="h-5 w-5 text-rose-500" />
        </div>
        ...
        <div v-else class="px-6 py-14 text-center">
          <BookHeart class="mx-auto h-10 w-10 text-slate-300" />
          <h3 class="mt-4 font-semibold text-slate-950">
            No wishlist activity yet
          </h3>
          <p class="mt-2 text-sm text-slate-500">
            Student course saves will appear here when they add courses to their
            wishlists.
          </p>
        </div>
      </section>
