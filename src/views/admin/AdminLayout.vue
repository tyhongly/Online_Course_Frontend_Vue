...
const displayName = computed(
  () => authStore.user?.username || authStore.user?.name || "Admin",
);
const goHome = () => {
  router.push("/");
};
...
<template>
  <div class="min-h-screen bg-light text-slate-900 md:overflow-hidden">
    <aside
      class="w-full border-b border-slate-200 bg-white md:fixed md:inset-y-0 md:left-0 md:z-30 md:flex md:w-72 md:flex-col md:border-b-0 md:border-r md:overflow-y-auto"
    >
