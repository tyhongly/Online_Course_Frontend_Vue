...
<template>
  <div class="mx-auto max-w-6xl space-y-6">
    ...
    >
        ...
        >
          <div>
            ...
            <p class="mt-2 text-sm leading-7 text-slate-600">
              Keep the courses you want to explore next close at hand.
            </p>
          </div>
      ...
      <section
        v-if="courses.length"
        class="border-t border-slate-200 px-6 py-7 sm:px-8"
      >
        <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          <article
            v-for="course in courses"
            :key="course.id"
            class="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_50px_-32px_rgba(15,23,42,0.14)]"
          >
            <router-link
              :to="`/course/${course.id}`"
              class="group block"
            >
              ...
              >
                ...
                <span
                  class="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-indigo-700"
                >
                  {{ course.category }}
                </span>
              </div>
              ...
              <div class="p-5">
                <h2
                  class="line-clamp-2 text-lg font-semibold leading-7 text-slate-950 group-hover:text-indigo-600"
                >
                  {{ course.title }}
                </h2>
                ...
                >
                  <span
                    v-if="course.type === 'document' || course.price === 0"
                  ...
                  <span
                    class="inline-flex items-center gap-1 text-sm font-semibold text-slate-600"
                  >
                    View course
      ...
      <section
        v-else
        class="border-t border-slate-200 px-6 py-10 text-center sm:px-8"
      >
        <div
          class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-rose-100 text-rose-600"
        >
          <Heart class="h-6 w-6" />
        </div>
        ...
        >
          Save courses from the catalog when you find something you want to
          learn later.
        </p>

        <router-link
          to="/courses"
          class="mt-7 inline-flex items-center gap-2 rounded-2xl bg-primary px-6 py-3 text-sm font-semibold text-white transition hover:bg-slate-800"
        >
          Browse courses
          <ArrowRight class="h-4 w-4" />
        </router-link>
